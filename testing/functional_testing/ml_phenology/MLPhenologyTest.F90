program MLPhenology
  ...
  ! ======================================
  ! test normalize_standardize_chain
  ...
  ! print the normalized and standardized data
  !print *, "raw    min/max: ", minval(lai), maxval(lai)
  print *, "raw historical LAI ", lai(1:60)
  !print *, "normalized LAI ", lai_std(1:60)
  !print *, "minmax LAI ", lai_mm(1:60)
  print *, "standardized LAI ", lai_stdmm(1:60)
  ! ======================================
  ...
  ! tmin, tmax, precip, rad, photoperiod, swvl1, doy, lai
  call run_tft_model(the_tft_torch_model, ta_min, ta_max, pr, sw, lai, soilm, doy, photo, doy_arr, out_data_tft)
  ...
  !-----------------------------------------------------------------------
    ...
    subroutine normalize_standardize_chain(                                     &
        ...
        implicit none
        real(r8), intent(in )  :: x(:)
        real(r8), intent(out) :: x_std(size(x))
        real(r8), intent(out) :: x_mm(size(x))
        real(r8), intent(out) :: x_stdmm(size(x))
    ...
    subroutine run_tft_model (the_tft_torch_model, ta_min, ta_max, pr, sw, lai, soilm, doy, photo, doy_arr, out_data_tft)
        ...
        real(r8),         intent(in) :: ta_min(:), ta_max(:), pr(:), sw(:), lai(:), soilm(:), doy(:), photo(:), doy_arr(10)
